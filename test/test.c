...
__attribute__((visibility("default"))) int generate_textures(state_t* state, float dt)
{
    ...
    /* texture atlas usage code */
    {
        ...
        texture_t tex_1 = tex_build(subtexture(&atlas,32,32,RED), noise(zero_to_one + 0.1));
        texture_t tex_2 = tex_build(subtexture(&atlas,32,32,BLUE));
        texture_t tex_3 = tex_build(subtexture(&atlas,32,32,GREEN));
        texture_t tex_4 = tex_build(subtexture(&atlas,32,32,VIOLET));
        texture_t tex_9 = tex_build(subtexture(&atlas,32,32,GREEN),
                                   noise(zero_to_one + 0.8), circle(16,16,(int)(zero_to_one*8),RED),
                                   rect(0,(int) (zero_to_one * 29),32,4, VIOLET), rect(0,32,32,4,RED),
                                   flip()
                                  );
        texture_t tex_5 = tex_build(subtexture(&atlas,32,32,YELLOW));
        texture_t tex_6 = tex_build(subtexture(&atlas,32,32,BROWN));
        texture_t tex_7 = tex_build(subtexture(&atlas,32,32,ANIMATED_COLOR));
        texture_t tex_8 = tex_build(subtexture(&atlas,32,32,CYAN));
        //texture_t tex_9 = tex_build(subtexture(&atlas,32,32,PURPLE));
